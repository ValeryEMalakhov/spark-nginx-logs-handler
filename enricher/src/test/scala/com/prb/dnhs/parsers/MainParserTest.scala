...
class MainParserTest extends mutable.Specification
  ...
  private val mainParser = new MainParser {
  ...
  val parsedLogString: RDD[Either[ErrorDetails, Row]] = mainParser.parse(testRddLogString)

  val validParsedLogString: RDD[Row] = parsedLogString.map(_.right.get)
